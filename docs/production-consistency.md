...
离线、心跳陈旧、取杯占位、设备忙或活动任务/HOLD 等阻塞时，有排队订单的派单请求保留并退避重试。Worker 启动首轮及此后每 30 秒为订单和任务均为 QUEUED、但缺少请求的设备补建请求；不绕过上述联锁，不直接发送制作命令。
...
迁移11包含活动任务唯一索引，12为裁决幂等记录，21为队列/库存水位，22为取杯联锁，23为执行 attempt，24增加 MQTT Inbox 恢复次数、下次恢复时间和待恢复索引。部署须运行全部未应用迁移，不应只按旧发布记录停在12或18。
