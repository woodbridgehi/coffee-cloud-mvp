...
> 本文为待执行的验收计划，阈值不是本次已测结果。2026-09-17按当前进度/队列行为修订。

## 目标模型

- 设备心跳 30 秒并随机错峰。
- 活跃设备按整体进度变化至少5%或经过5秒任一条件发送task.progress；短配方可更频繁，负载模型须按实际时长统计。
- 生命周期、命令结果、失败与库存事实事件不合并。
- 制作状态使用SSE；排队SSE每5秒刷新队列，匿名旁观另有4秒查询，负载不能只统计进度事件。
