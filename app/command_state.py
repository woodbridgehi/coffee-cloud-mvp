...
LEGAL_TRANSITIONS: dict[str, frozenset[str]] = {
    # UNKNOWN from CREATED is required when expiry finds publication evidence in the
    # outbox: the physical outcome is unknowable and must not become terminal EXPIRED.
    # Authenticated device results may beat the gateway's PUBACK HTTP receipt.
    CREATED: frozenset({DELIVERING, PUBLISHED, ACKED, EXECUTING, SUCCEEDED, REJECTED, FAILED, CANCELLED, EXPIRED, UNKNOWN}),
    ...
    ACKED: frozenset({EXECUTING, SUCCEEDED, REJECTED, FAILED, CANCELLED, UNKNOWN}),
