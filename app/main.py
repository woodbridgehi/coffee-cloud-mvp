...
from .protocol import (
    ...
    CredentialRotationRequest, DeviceEvent, Heartbeat, Snapshot, TaskAck,
...
def heartbeat(device_id: str, payload: Heartbeat, identity: DeviceIdentity) -> dict[str, Any]:
    ...
    with database.connect() as connection:
        ...
        if existing:
            ...
            return {"ok": True, "duplicate": True, "disposition": existing["disposition"], "receivedAt": iso(received_at), "qrUrl": settings.order_qr_url}
    ...
    return {"ok": True, "duplicate": False, "disposition": disposition, "receivedAt": iso(received_at), "qrUrl": settings.order_qr_url}
    ...
    with database.connect() as connection:
        ...
        for row in rows:
            ...
            if row["expires_at"] and row["expires_at"] <= utc_now() and row["status"] not in TERMINAL_STATES:
                transition_command(connection, row, EXPIRED, "cloud", reason="command expired before delivery")
                continue
...
def events(device_id: str, payload: DeviceEvent, identity: DeviceIdentity) -> dict[str, Any]:
    ...
    with database.connect() as connection:
        ...
        if existing:
            ...
            return {"ok": True, "duplicate": True, "commandTransition": command_transition}
        connection.execute(
        ...
        command_transition = reconcile_command_event(connection, identity["id"], body, payload.type)
    return {"ok": True, "duplicate": False, "commandTransition": command_transition}
...
def display_config(device_id: str, identity: DeviceIdentity) -> dict[str, Any]:
    return {"qrUrl": settings.order_qr_url}
...
def admin_device(identifier: str, _: AdminAuth) -> dict[str, Any]:
    ...
    return {**admin_device_payload({**row, "heartbeat_count": heartbeat_count, "event_count": event_count, "command_count": command_count}), "snapshots": {item["snapshot_type"]: {"version": item["version"], "receivedAt": iso(item["received_at"])} for item in snapshot_rows}}
...
<script>
let adminToken='',devices=[],selectedId='',refreshTimer=null;
...
async function loadDevices(){try{const data=await api('/api/v1/admin/devices');devices=Array.isArray(data.devices)?data.devices:[];el('server-time').textContent='服务时间：'+time(data.serverTime);el('last-refresh').textContent='每 10 秒自动刷新 · '+new Date().toLocaleTimeString('zh-CN',{hour12:false});el('total').textContent=devices.length;el('online').textContent=devices.filter(d=>d.online).length;el('offline').textContent=devices.filter(d=>!d.online&&d.hasEverConnected).length;el('never').textContent=devices.filter(d=>!d.hasEverConnected).length;renderTable();if(selectedId){const selected=devices.find(d=>d.deviceId===selectedId);if(selected)renderDetail(selected)}}catch(error){if(adminToken)setMessage('login-error','加载失败：'+error.message)}}
...
function renderDetail(device){const detail=el('detail');detail.replaceChildren();const title=document.createElement('h3');title.textContent=device.deviceId+' · '+text(device.storeId);detail.append(title);const grid=document.createElement('div');grid.className='detail-grid';const fields=[['连接',device.online?'在线':device.hasEverConnected?'离线':'待激活'],['生命周期',device.lifecycleStatus],['实例',device.instanceId],['序列号',device.serialNumber],['软件版本',device.softwareVersion],['最近心跳',time(device.lastHeartbeatAt)],['最后上线',time(device.lastConnectedAt)],['消息统计','心跳 '+device.heartbeatCount+' · 事件 '+device.eventCount+' · 命令 '+device.commandCount]];for(const [label,value] of fields){const item=document.createElement('div');item.className='detail-item';const key=document.createElement('label');key.textContent=label;const val=document.createElement('strong');val.textContent=text(value);item.append(key,val);grid.append(item)}detail.append(grid);const pre=document.createElement('pre');pre.textContent=JSON.stringify({reportedStatus:device.reportedStatus,lastErrorSummary:device.lastErrorSummary,activeBootId:device.activeBootId,lastSequence:device.lastSequence},null,2);detail.append(pre)}
